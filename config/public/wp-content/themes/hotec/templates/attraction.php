 ...
 $p_layout =  strtolower($st_page_options['attraction_layout']);
 if($p_layout!='full'){
    $p_layout ='half';
 }
 ?>
    ...
    <?php if($p_layout=='half'){ ?>
        ...
        <div class="project-right-detail four columns">
            <div class="p-desc">
                <h3 class="spd-heading"><?php _e('Short Description','smooththemes'); ?></h3>
                <?php  echo apply_filters('the_content',$post->post_excerpt);  ?>
            </div>
             <div class="single-portfolio-details">
                  <?php include(dirname(__FILE__).'/attraction-details.php'); ?>
            </div>
        </div>
        <div class="clear"></div>
    ...
    <?php } ?>
        ...
        <div class="twelve columns b20 text-content">
            <h3 class="spd-heading"><?php _e('Full Description','smooththemes'); ?></h3>
            <?php the_content(); ?>
        </div>
        ...
        <?php
        // recent portfolio 
        $term_list = wp_get_post_terms($post->ID, 'attraction_tag', array("fields" => "term_id"));
        $ids=array();
        foreach($term_list as $term){
            $ids[]= $term->term_id;
        }
        echo do_shortcode('[attraction numpost="3" title="'.__('Related Attractions','smooththemes').'"  filter_type="custom" exclude="'.$post->ID.'" num_col="3" cats="'.join(',',$ids).'" ]');
        ?>
        </div>
